...
pub pure fn to_owned(s: &str) -> ~str {
...
pub mod raw {
     ...
     * Takes a bytewise (not UTF-8) slice from a string.
     *
     * Returns a new substring from [`begin`..`end`).
     *
     * # Failure
     *
     * If begin is greater than end.
...
impl StrSlice for &'self str {
    ...
    #[inline]
    pure fn to_owned(&self) -> ~str { to_owned(*self) }
...
mod tests {
    ...
    fn test_unsafe_slice() {
        unsafe {
            ...
            fail_unless!(half_a_million_letter_a() ==
                raw::slice_bytes(a_million_letter_a(), 0u, 500000).to_owned());
        }
    }
    ...
    fn test_slice() {
        ...
        let a_million_X = a_million_letter_X();
        fail_unless!(half_a_million_letter_X() ==
            slice(a_million_X, 0u, 3u * 500000u).to_owned());
    }
