...
impl float: num::Round {
    #[inline(always)]
    pure fn round_to_integer(&self, mode: num::RoundModeInteger) -> float {
        match mode {
            num::RoundDown                           => f64::floor(*self as f64) as float,
            num::RoundUp                             => f64::ceil(*self as f64) as float,
            num::RoundToZero   if is_negative(*self) => f64::ceil(*self as f64) as float,
            num::RoundToZero                         => f64::floor(*self as f64) as float,
            num::RoundFromZero if is_negative(*self) => f64::floor(*self as f64) as float,
            num::RoundFromZero                       => f64::ceil(*self as f64) as float
        }
    }
