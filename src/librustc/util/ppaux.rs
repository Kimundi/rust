...
pub fn vec_map_to_str<T>(ts: &[T], f: |t: &T| -> ~str) -> ~str {
    let tstrs = ts.iter().map(f).collect::<~[~str]>();
...
pub fn ty_to_str(cx: ctxt, typ: t) -> ~str {
    ...
    fn push_sig_to_str(cx: ctxt,
                       ...
                       sig: &ty::FnSig) {
        s.push_char(bra);
        let strs = sig.inputs.iter().map(|a| fn_input_to_str(cx, *a)).collect::<~[~str]>();
    ...
    return match ty::get(typ).sty {
      ...
      ty_tup(ref elems) => {
        let strs = elems.iter().map(|elem| ty_to_str(cx, *elem)).collect::<~[~str]>();
        ~"(" + strs.connect(",") + ")"
      }
      ty_closure(ref f) => {
          closure_to_str(cx, f)
